...
//                ->join('LEFT JOIN __PROJECT__ AS p ON p.pro_id=t.pro_id')
    ...
    //过滤表单列表的可看人员
    public function filterMemberToFrom($proId, $proLevel, $adminId, $type = null,$formType=null,$dbName=null)
    {
        $map['_string'] = "substr(`pro_level_now`,1,2)=$proLevel and `pj_id`=$proId";

        if ($type) {
            //  ->join('LEFT JOIN __WORKFLOW_LOG__ AS wl ON  w.wf_id= wl.wf_id')
            $list = D('PjWorkflow')->alias('pj')
                ->join("LEFT JOIN __WORKFLOW_LOG__ AS wl ON pj.wf_id=wl.wf_id")
                ->join("LEFT JOIN $dbName AS fp ON fp.wf_id=pj.wf_id")
                ->where("substr(pj.pro_level_now,1,2)=$proLevel and pj.pj_id=$proId and wl.pro_author=$adminId and (wl.pro_state=2 or wl.pro_state=3) and fp.form_type='$formType'")
                ->field('fp.id,fp.addtime,fp.wf_id')
                ->select();

            /*            $pjId=D('PjWorkflow')->where($map)->getField('wf_id',true);
                        $pjIdList=implode(',',$pjId);
                        $list=D('WorkflowLog')->where("`wf_id` in($pjIdList) and `pro_author`=%d and (`pro_state`=2 or `pro_state`=3)",array($adminId))->getField('wf_id',true);*/
        } else {
