...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        var $fullCalendar = $box.find('[data-toggle="fullCalendar"]')
        
        $fullCalendar.each(function(){
            var $element = $(this);
            var options  = $element.data();
            var url = '/Admin/Calendar/myCalendar.html';
		...
		header: {
			...
			right: 'month,agendaWeek,agendaDay'
                ...
                height: 500,
		events: '/Admin/Calendar/myCalendar.html',
		dayClick: function(date, allDay, jsEvent, view) {
			var selDate =$.fullCalendar.formatDate(date,'yyyy-MM-dd');
                        $(this).dialog({
                            id:'mydialog',
                            url:'/Admin/Calendar/add/date/'+selDate,
                            title:'添加事件'
                        });
                },
                eventClick: function(calEvent, jsEvent, view) {
                    $(this).dialog({
                        id:'mydialog',
                        url:'/Admin/Calendar/edit/id/'+calEvent.id,
                        title:'编辑事件'
                    });
                }
            });
